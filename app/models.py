...
class Comment(models.Model):
    ...
    date = models.DateTimeField(default = datetime.now(), db_index =True, verbose_name="Дата добавления комментария")  # Дата добавления комментария
    author = models.ForeignKey(User, on_delete=models.CASCADE, verbose_name="Автор комментария")  # Автор комментария
    post = models.ForeignKey('Blog', on_delete=models.CASCADE, verbose_name="Статья комментария")  # Статья блога, к которой относится комментарий
#Метод класса
def __str__(self):
    return 'Комментарий %d %s k %s' % (self.id, self. autor, self.post)
#Метаданные - вложенный класс, который задает дополнительные параметры модели:
class Meta:
    db_table = "Comment"
    ordering = ["-date"]
